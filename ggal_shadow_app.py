...
from ggal_shadow import db, run_cycle

app=FastAPI(title="PMSF-X Nano — GGAL Shadow", version="0.1.0")
...
@app.get("/health")
def health(): return {"ok":True,"service":"ggal-shadow","mode":"SHADOW","triggers_enabled":False}
...
def shadow():
    ...
    if not latest:
        result=run_cycle()
        return {"forecast":result["model"],"health":{"market":"HEALTHY","risk":"HEALTHY"},"validation":result["model"]}
    return {"forecast":dict(latest),"health":{"market":"HEALTHY","risk":"HEALTHY"},"validation":{"accuracy_oos":latest["accuracy_oos"],"brier_oos":latest["brier_oos"],"sample_count":latest["sample_count"],"type":"chronological_oos_holdout"},"latest_inputs":{"ggal":dict(m) if m else None,"embi":dict(r) if r else None}}
